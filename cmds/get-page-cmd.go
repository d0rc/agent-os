...
func ProcessPageRequests(request []GetPageRequest, storage *Storage) (*ServerResponse, error) {
	...
	for idx, pr := range request {
		...
		go func(pr GetPageRequest, ch chan *GetPageResponse) {
			...
			pageResponse, err := processPageRequest(pr, storage)
			if err != nil {
...
func processPageRequest(pr GetPageRequest, storage *Storage) (*GetPageResponse, error) {
	// let's read all the pages we've seen from this url and pick latest
	cachedPage := make([]PageCacheRecord, 0, 1)
	err := storage.db.GetStructsSlice("query-page-cache", &cachedPage, pr.Url)
	if err != nil {
	...
	if len(cachedPage) > 0 {
		...
		if time.Since(cachedPage[0].CreatedAt).Seconds() < float64(pr.MaxAge) {
			...
			_, err = storage.db.Exec("make-page-cache-hit", cachedPage[0].Id)
			if err != nil {
				storage.lg.Error().Err(err).Msgf("error marking page cache hit: %v", cachedPage[0].Id)
			}

			return translateCacheRecordToClientResponse(&cachedPage[0], storage), nil
		}
	}
	...
	for retryCounter := 0; retryCounter < pr.MaxRetries; retryCounter++ {
		pageCacheRecord, err = downloadPage(pr)
		if err != nil {
			storage.lg.Warn().Err(err).
	...
	res, err := storage.db.Exec("save-page-cache-record",
		pageCacheRecord.Url,
	...
	return translateCacheRecordToClientResponse(pageCacheRecord, storage), nil
}

func translateCacheRecordToClientResponse(cachedPage *PageCacheRecord, storage *Storage) *GetPageResponse {
	mdBody, err := renderMarkdown(string(cachedPage.RawContent))
	if err != nil {
		storage.lg.Error().Err(err).Msgf("error rendering markdown for page cache id: %v",
			cachedPage.Id)
	}
...
func downloadPage(pr GetPageRequest) (*PageCacheRecord, error) {
	...
	finalUrl := fmt.Sprintf("https://api.crawlbase.com/?token=%s&url=", settings.CrawlBaseToken) + escapedUrl
