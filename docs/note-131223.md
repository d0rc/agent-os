...
With the resulting infrastructure, we want to try launching agents that perform "paid" assignments for each other, as well as several 'proxy agents' offering tasks of varying cost and complexity. These agents will provide a reward only if the task is solved correctly. 

Moreover, there is a substantial theoretical foundation suggesting that it's not necessarily required to attempt RL (Reinforcement Learning) for LLMs (Large Language Models), as embedding vectors carry enough information[^EMB-RECON] for partial reconstruction of the original text on one hand, and structural semantic information on the other. We would like to try creating smaller models capable of navigating to some extent in the embeddings space.

## Mining synthetic training data
